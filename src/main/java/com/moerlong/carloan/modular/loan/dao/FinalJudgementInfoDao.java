package com.moerlong.carloan.modular.loan.dao;

import java.util.Map;
import java.util.List;
import com.moerlong.carloan.modular.loan.entity.FinalJudgementInfo;

public interface FinalJudgementInfoDao {

	/**
	 * 保存
	 * @param entity
	 */
	public void save(FinalJudgementInfo entity);

	/**
	 * 删除
	 * @param id
	 */
	public void delete(Long id);
	
	/**
	 * 逻辑删除
	 * @param id
	 */
	public void deleteLogic(Long id);
	
	/**
	 * 更新
	 * @param entity
	 */
	public void update(FinalJudgementInfo entity);
	
	/**
	 * 更新非空字段
	 * @param entity
	 */
	public void updateWithOutNull(FinalJudgementInfo entity);

	/**
	 * 按id查询
	 * @param id
	 * @return
	 */
	public FinalJudgementInfo selectById(Long id);
	
	public FinalJudgementInfo selectByApplyId(Long id);
	
	/**
	 * 查询所有
	 * @return
	 */
	public List<FinalJudgementInfo> listAll();
	
	/**
	 * 分页查询 
	 * @param param	查询参数
	 * @return
	 */
	public List<FinalJudgementInfo> selectPage(Map<String,Object> param);

}

