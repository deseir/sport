...
public class CarBodyConfigController {

	private final Logger log = LoggerFactory.getLogger(CarBodyConfigController.class);

	@Autowired
	CarBodyConfigService service;
	@Autowired
	ApplyInfoService applyInfoService;

	@ApiOperation(value = "保存或更新")
	@ApiImplicitParam(paramType = "body", name = "entity", required = true, dataType = "CarBaseParams", value = "明细")
	@RequestMapping(value = "/carBodyConfig/saveOrUpdate", method = RequestMethod.POST)
	@ResponseBody
	public Object saveOrUpdate(CarBodyConfig entity) {
	...
	@RequestMapping(value = "/carBodyConfig/findByApplyId", method = {RequestMethod.POST,RequestMethod.GET})
	@ResponseBody
	public Object findByApplyId(@RequestParam Map<String,Object> param) {
		Map<String, Object> res = new HashMap<>();
		try {
			Long applyId = Long.valueOf(param.get("applyId").toString());
			CarBodyConfig carBodyConfig = service.selectByApplyId(applyId);
			res.put("data",carBodyConfig);
			res.put("status", carBodyConfig!=null?0:1);
			res.put("errMsg", "操作成功");
		} catch (Throwable e) {
			this.log.error(e.getMessage(), e);
			res.put("status", 1);
