;(function ($) {
  $.fn.frame = function (options) {
    ...
    this.each(function () {
      ...
      content.height($(window).height() - ((options.header) + (options.bottom))).css({
        "width": $(window).width() - (options.menu),
        "margin-top": options.header,
        "margin-left": options.menu,
        "position": "relative"
      });
      //当窗口发生改变是触发
      ...
      menunav.on("click", ".submenu a, .shop_index a", function () {
        if ($(this).attr('title')) {
          ...
          if (!bStop) {
            ...
            creatIframe(_href, _titleName, _link);
            min_titleList();
          }
          else {
      ...
      function creatIframe(href, titleName, link) {
        var topWindow = $(window.parent.document);
        var show_nav = topWindow.find('#min_title_list');
        // var id_name=$(".show_iframe").each(function(i){$(this).attr('id',"Sort_link_"+i);});
        show_nav.find('li').removeClass("active");
        // var iframe_box = topWindow.find('#iframe_box');
        show_nav.append('<li class="active"><a href="javascript:void(0);" routerLink="'+link+'"><span title="' + href + '">' + titleName + '</span><em class="close_icon"></em></a></li>');
        tabNavallwidth();
