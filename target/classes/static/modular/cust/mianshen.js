...
$(document).ready(function(){
	 ...
	 Feng.initValidator("judeicialName", saveOrUpdates.validateFields);
});
...
//	                },
         ...
         tdScoreAttachUrlmImg:{
             validators: {
                 notEmpty: {
                     message: '同盾贷前审核，附件必须上传'
                 },
             }
		 },
...
function saveOrUpdate(opt){
    ...
    if (opt=='judicial'){//司法认证
        ...
        var applyId = $("#applyId").val();
        var pamType =$("#pamType").val();
        var applyAmount=$("#applyAmount").val();//本次拟贷款本金
        var applyPeriod=$("#applyPeriod").val();//本次拟贷款期限
    ...
    }else if (opt=='gongjie'){//共借人
        var marryStatus = $("#marryStatus").val();
        if(marryStatus==""||marryStatus==null||marryStatus==undefined){
        	Feng.info("请选择婚姻状况");
		}
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if(status=='0'){
                ...
                $("#applyAmount").val(data.data.applyAmount);
                $("#applyPeriod").val(data.data.applyPeriod);
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if(status=='0'){
                ...
                $("#gjrmobile").val(data.data.mobile);
            ...
            if(status=='0'){
                ...
                $.each(data.data,function(i){
                    html+="<tr align='center'><td><input type='checkbox' value='"+data.data[i].id+"' name='financeChecked'/></td>"
                        ...
                        +"<td>"+(data.data[i].propertyName==""?"--":data.data[i].propertyName)+"</td>"
            ...
            if(status=='0'){
                ...
                $.each(data.data,function(i){
                    html+="<tr align='center'><td><input type='checkbox' value='"+data.data[i].id+"' name='loanChecked'/></td><td>"+data.data[i].loanOrg+"</td>"
                        ...
                        +"<td>"+data.data[i].loanTypeName+"</td>"
                        ...
                        +"<td>"+data.data[i].accountStatusName+"</td>"
                        +"<td>"+data.data[i].fiveClassStatusName+"</td>"
            ...
            if(status=='0'){
                ...
                $.each(data.data,function(i){
                    html+="<tr align='center'><td><input type='checkbox' value='"+data.data[i].id+"' name='cardChecked'/></td><td>"+data.data[i].cardOrg+"</td>"
                        ...
                        +"<td>"+data.data[i].cardTypeName+"</td>"
                        +"<td>"+data.data[i].accountStatusName+"</td>"
            ...
            if(status=='0'){
                ...
                $.each(data.data,function(i){
                    html+="<tr align='center'><td><input type='checkbox' value='"+data.data[i].id+"' name='bussQueryChecked'/></td>"
                        ...
                        +"<td>"+data.data[i].queryReasonName+"</td>"
                        +"<td>"+data.data[i].remark+"</td></tr>";
                });
...
function  calcTingnuoPrice() {
    ...
    //var result = Math.round(Number(tsingnuoPrice)*100/100);
...
function setIncomeConfirmAmount() {
   ...
   //收入认定基数
    ...
    var  incomeType=$("#incomeType").val();
    var incomeConfirmAmount = 0;
    //0-按揭房贷方案/1-非按揭房贷方案/
    if(incomeType==0||incomeType==1){
        incomeConfirmAmount = Number(incomeAmount)*8;
    }if(incomeType==2){//2-全款房方案
        incomeConfirmAmount = (Number(incomeAmount)*0.7/360+Number(incomeAmount)*0.7*0.42/100)*8;
    }else  if(incomeType==3){//3-寿险保单方案
        incomeConfirmAmount = Number(incomeAmount)*6;
    }else if(incomeType==4){// 4-他行车贷方案
        incomeConfirmAmount = Number(incomeAmount)*5;
    }else if(incomeType==5){//5-车主为自雇人士 - 流水结息方案
        incomeConfirmAmount = Number(incomeAmount)*1100/3;
    }else if(incomeType==6){//6-为公积金方案
        incomeConfirmAmount = Number(incomeAmount)*3;
    }else if(incomeType==7){//7-为打卡工资方案
        incomeConfirmAmount = Number(incomeAmount)*2;
    }else if(incomeType==8){//8-车主为自由职业 - 流水结息方案
        incomeConfirmAmount = Number(incomeAmount)*1100/6;
    }
...
function showGjr() {
    var applyId = $("#applyId").val();
    var interfaceAddress = $("#interfaceAddress").val();
    window.location.href="/mianshen/showGjrMs?applyId="+applyId+"&interfaceAddress="+interfaceAddress;
}
...
function  setDti() {
	...
	//本次拟贷款期限
    var applyPeriod = $("#applyPeriod").val();
	//本次拟月还款本金=本次拟贷款本金/本次拟贷款期限
	var monthMonth = Number(applyAmount)/Number(applyPeriod);

debugger;
	//本次拟月还款利率 = 本次拟贷款本金 * 0.83%
	var monthLlv = Number(applyAmount)*0.83/100;

	//本次拟服务费 = 本次拟贷款本金 * 3%/本次拟贷款期限
	var castFee = (Number(applyAmount)*3/100)/Number(applyPeriod);

	//流量费
	var liuliangFee = 100;

    //本次拟贷月供 = 本次拟月还款本金+本次拟月还款利率+流量费+本次拟服务费
    var loanAmountTmp = Number(monthMonth)+Number(monthLlv)+Number(liuliangFee)+Number(castFee);
...
debugger;
    if(incomeConfirmAmount !=0&&incomeConfirmAmount !=null &&incomeConfirmAmount !="" && creditLoanMonthAmount !=null && creditLoanMonthAmount!=""){
        var totalLoan = Number(creditLoanMonthAmount)+Number(loanAmountTmp);
...
//面审主管查看共借人信息
function  showGjrDetail() {
    var applyId = $("#applyId").val();
    var interfaceAddress = $("#interfaceAddress").val();
    var flag = $("#flag").val();
    window.location.href="/mianshen/showGjrMsDetail?applyId="+applyId+"&interfaceAddress="+interfaceAddress+"&flag="+flag;
}
//面审提交
function msSubmit() {
    var applyId = $("#applyId").val();
    var result = $("#interviewResult").val();
    var resultTip = $("#overview").val();
    var interfaceAddress = $("#interfaceAddress").val();
...
saveOrUpdates.validate7= function () {
    $('#judeicialName').data("bootstrapValidator").resetForm();
    $('#judeicialName').bootstrapValidator('validate');
    return $("#judeicialName").data('bootstrapValidator').isValid();
}
