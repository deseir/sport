...
$(document).ready(function(){
    ...
    Feng.initValidator("newAddFamily", custNqData.validateFields);
    Feng.initValidator("updateInfo", custNqData.validateFields);
});
...
//		 },
         ...
         familyBookSubInfoRelationship2:{
             validators: {
                 notEmpty: {
                     message: '申请人与关联人关系不能为空'
                 },
...
custNqData.validate10 = function () {
    $('#newAddFamily').data("bootstrapValidator").resetForm();
    $('#newAddFamily').bootstrapValidator('validate');
    return $("#newAddFamily").data('bootstrapValidator').isValid();
}
custNqData.validate11 = function () {
    $('#updateInfo').data("bootstrapValidator").resetForm();
    $('#updateInfo').bootstrapValidator('validate');
    return $("#updateInfo").data('bootstrapValidator').isValid();
}
...
custNqData.collectData = function(flag) {
    if(flag == 1){
        ...
        if($("#loanUsage5").is(':checked')){
            var loanUsage6 = $("#loanUsage6").val();
            if ( loanUsage6=="" || loanUsage6==null || loanUsage6==undefined) {
                Feng.info("请填写其他用途")
                return false
            }
        }
        ...
        if(fileObj!=undefined) {
		 ...
		 if($("#idFrontPhotoUrl").val()==''||$("#idFrontPhotoUrl").val()==undefined||$("#idFrontPhotoUrl").val()==null){
		...
		if($("#idBackPhotoUrl").val()==''||$("#idBackPhotoUrl").val()==undefined||$("#idBackPhotoUrl").val()==null){
            Feng.info("请上传身份证反面照片！")
            return false;
		}
        //身份证基本信息
      ...
      /*  familyBookSubInfolist.push(FamilyBookSubInfos);*/
        ...
        if($("#marryStatus").val()=="0"||$("#marryStatus").val()=="2"){//已婚或再婚
            ...
            if($("#spouseidFrontPhotoUrl").val()==''||$("#spouseidFrontPhotoUrl").val()==undefined||$("#spouseidFrontPhotoUrl").val()==null){
                Feng.info("请上传身份证正面照片！")
                return false;
            }
            ...
            if($("#marryPhotoUrl").val()==''||$("#marryPhotoUrl").val()==undefined||$("#marryPhotoUrl").val()==null){
                Feng.info("请上传结婚照片！")
                return false;
            }
            ...
            if($("#spouseidBackPhotoUrl").val()==''||$("#spouseidBackPhotoUrl").val()==undefined||$("#spouseidBackPhotoUrl").val()==null){
                Feng.info("请上传身份证反面照片！")
                return false;
            }
            ...
            if($("#divorcePhotoUrl").val()==''||$("#divorcePhotoUrl").val()==undefined||$("#divorcePhotoUrl").val()==null){
                Feng.info("请上传离婚证照片！")
                return false;
            }
            ...
            if($("#deathCertPhotoUrl").val()==''||$("#deathCertPhotoUrl").val()==undefined||$("#deathCertPhotoUrl").val()==null){
                Feng.info("请上传死亡证明照片！")
                return false;
            }
        ...
        if($("#registerPhotoUrl1").val()==''||$("#registerPhotoUrl1").val()==undefined||$("#registerPhotoUrl1").val()==null){
        ...
        if($("#vehicleFrontPhoto").val()==''||$("#vehicleFrontPhoto").val()==undefined||$("#vehicleFrontPhoto").val()==null){
            Feng.info("请上传行驶证正面照片！")
            return false;
        }
        ...
        if($("#vehicleBackPhoto").val()==''||$("#vehicleBackPhoto").val()==undefined||$("#vehicleBackPhoto").val()==null){
            Feng.info("请上传行驶证背面照片！")
            return false;
        }
        var fileObj = document.getElementById("driverFrontPhoto1").files[0];
         ...
         if($("#driverFrontPhoto").val()==''||$("#driverFrontPhoto").val()==undefined||$("#driverFrontPhoto").val()==null){
        ...
        if($("#driverBackPhoto").val()==''||$("#driverBackPhoto").val()==undefined||$("#driverBackPhoto").val()==null){
...
function newAddFamilyBookSubInfo(){
	if($("#familyBookInfoId").val()==undefined||$("#familyBookInfoId").val()==""){
		Feng.info("请先保存户口本信息!")
	}else{
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/familyBookInfo/familyBookInfoPage?familyBookInfoId='+$("#familyBookInfoId").val()+"&applyId="+$("#applyId").val()
        });
...
function AddFamilyBookSubInfo(){
    ...
    //     var str1='<tr  style="background-color:#FFF;padding: 1px;box-shadow: 0 0 2px #FFF inset;margin-top: 3px"><td type="hidden" style="display:none" ></td><td style="text-align: center">'+$("#familyBookSubInfoRelationship1").val()+'</td><td style="text-align: center">'+$("#familyBookSubInfoName1").val()+'</td><td style="text-align: center">'+$("#familyBookSubInfoCertId1").val()+'</td><td style="text-align: center">男</td><td style="text-align: center"><button class="btn btn-primary" onclick="delFamilyBookSubInfo(this);"><i class="fa fa-remove"></i>&nbsp;&nbsp;&nbsp;&nbsp;删除</button></td><td style="text-align: center"><button class="btn btn-primary" onclick="updFamilyBookSubInfo(this);"><i class="fa fa-edit"></i>&nbsp;&nbsp;&nbsp;&nbsp;修改</button></td><td style="text-align: center;display: none">'+$("#bookPhotoUrl2").val()+'</td></tr>';
    // }else{
    //     var str1='<tr  style="background-color:#FFF;padding: 1px;box-shadow: 0 0 2px #FFF inset;margin-top: 3px"><td type="hidden" style="display:none" ></td><td style="text-align: center">'+$("#familyBookSubInfoRelationship1").val()+'</td><td style="text-align: center">'+$("#familyBookSubInfoName1").val()+'</td><td style="text-align: center">'+$("#familyBookSubInfoCertId1").val()+'</td><td style="text-align: center">女</td><td style="text-align: center"><button class="btn btn-primary" onclick="delFamilyBookSubInfo(this);"><i class="fa fa-remove"></i>&nbsp;&nbsp;&nbsp;&nbsp;删除</button></td><td style="text-align: center"><button class="btn btn-primary" onclick="updFamilyBookSubInfo(this);"><i class="fa fa-edit"></i>&nbsp;&nbsp;&nbsp;&nbsp;修改</button></td><td style="text-align: center;display: none">'+$("#bookPhotoUrl2").val()+'</td></tr>';
    // }
    var ajax = new $ax(Feng.ctxPath + "/familyBookSubInfo/saveFamilyBookSubInfo", function (data) {
        Feng.success("新增成功!");
    }, function (data) {
        Feng.error("新增失败!" + data.responseJSON.message + "!");
    });
    ajax.set("bookId",$("#SubInfoId").val());
    ajax.set("familyBookSubInfoRelationship1",$("#familyBookSubInfoRelationship1").val());
    ajax.set("familyBookSubInfoName1",$("#familyBookSubInfoName1").val());
    ajax.set("familyBookSubInfoCertId1",$("#familyBookSubInfoCertId1").val());
    ajax.set("familyBookSubInfoSex1",$("#familyBookSubInfoSex1").val());
    ajax.set("bookPhotoUrl2",$("#bookPhotoUrl2").val());
    ajax.start();
   // parent.$("#newAddFamilyBookSubInfoTr").append(str1);
	window.parent.custliclick();
...
function AddFamilyBookSubInfos(){
   ...
   /* if($("#familyBookSubInfoSex2").val()==0){
        var str1='<tr  style="background-color:#FFF;padding: 1px;box-shadow: 0 0 2px #FFF inset;margin-top: 3px"><td type="hidden" style="display:none" >'+$("#familyBookSubInfoId3").val()+'</td><td style="text-align: center">'+$("#familyBookSubInfoRelationship2").val()+'</td><td style="text-align: center">'+$("#familyBookSubInfoName2").val()+'</td><td style="text-align: center">'+$("#familyBookSubInfoCertId2").val()+'</td><td style="text-align: center">男</td><td style="text-align: center"><button class="btn btn-primary" onclick="delFamilyBookSubInfo(this);"><i class="fa fa-remove"></i>&nbsp;&nbsp;&nbsp;&nbsp;删除</button></td><td style="text-align: center"><button class="btn btn-primary" onclick="updFamilyBookSubInfo(this);"><i class="fa fa-edit"></i>&nbsp;&nbsp;&nbsp;&nbsp;修改</button></td><td style="text-align: center;display: none">'+$("#bookPhotoUrl22").val()+'</td></tr>';
    }else{
        var str1='<tr  style="background-color:#FFF;padding: 1px;box-shadow: 0 0 2px #FFF inset;margin-top: 3px"><td type="hidden" style="display:none" >'+$("#familyBookSubInfoId3").val()+'</td><td style="text-align: center">'+$("#familyBookSubInfoRelationship2").val()+'</td><td style="text-align: center">'+$("#familyBookSubInfoName2").val()+'</td><td style="text-align: center">'+$("#familyBookSubInfoCertId2").val()+'</td><td style="text-align: center">女</td><td style="text-align: center"><button class="btn btn-primary" onclick="delFamilyBookSubInfo(this);"><i class="fa fa-remove"></i>&nbsp;&nbsp;&nbsp;&nbsp;删除</button></td><td style="text-align: center"><button class="btn btn-primary" onclick="updFamilyBookSubInfo(this);"><i class="fa fa-edit"></i>&nbsp;&nbsp;&nbsp;&nbsp;修改</button></td><td style="text-align: center;display: none">'+$("#bookPhotoUrl22").val()+'</td></tr>';
    }*/
    var ajax = new $ax(Feng.ctxPath + "/familyBookSubInfo/saveFamilyBookSubInfo", function (data) {
        Feng.success("修改成功!");
    }, function (data) {
    ...
    ajax.set("id",$("#familyBookSubInfoId3").val());
    ajax.set("familyBookSubInfoRelationship2",$("#familyBookSubInfoRelationship1").val());
    ajax.set("familyBookSubInfoName2",$("#familyBookSubInfoName1").val());
    ajax.set("familyBookSubInfoCertId2",$("#familyBookSubInfoCertId1").val());
    ajax.set("familyBookSubInfoSex2",$("#familyBookSubInfoSex1").val());
    ajax.set("bookPhotoUrl22",$("#bookPhotoUrl22").val());
    ajax.start();
    window.parent.custliclick();
   // parent.$("#newAddFamilyBookSubInfoTr").append(str1);
    parent.layer.close(window.parent.layerIndex);
...
function AddCarInfos(){
    ...
    var ajax = new $ax(Feng.ctxPath + "/carTransferInfo/saveCarTransferInfo", function (data) {
        Feng.success("修改成功!");
    }, function (data) {
        Feng.error("修改失败!" + data.responseJSON.message + "!");
    });
    ajax.set("id",$("#carTransferInfoId3").val());
    ajax.set("carTransferInfoName",$("#carTransferInfoName2").val());
    ajax.set("carTransferInfoCertId",$("#carTransferInfoCertId2").val());
    ajax.set("carTransferInfoGetType",$("#carTransferInfoGetType2").val());
    ajax.set("carTransferInfoRegDate",$("#carTransferInfoRegDate2").val());
    ajax.start();
    window.parent.carInfoVoLiClick();
        //var str1='<tr style="background-color:#FFF;padding: 1px;box-shadow: 0 0 2px #FFF inset;margin-top: 3px"><td type="hidden" style="display:none" >'+$("#carTransferInfoId3").val()+'</td><td style="text-align: center">'+$("#carTransferInfoName2").val()+'</td><td style="text-align: center">'+$("#carTransferInfoCertId2").val()+'</td><td style="text-align: center">'+$("#carTransferInfoGetType2").val()+'</td><td style="text-align: center">'+$("#carTransferInfoRegDate2").val()+'</td><td style="text-align: center"><button class="btn btn-primary" onclick="delCarTransferInfo(this);"><i class="fa fa-remove"></i>&nbsp;&nbsp;&nbsp;&nbsp;删除</button></td><td style="text-align: center"><button class="btn btn-primary" onclick="updCarTransferInfo(this);"><i class="fa fa-edit"></i>&nbsp;&nbsp;&nbsp;&nbsp;修改</button></td></tr>';
...
function AddCarMortgageInfos(){
   ...
   /* var str1='<tr style="background-color:#FFF;padding: 1px;box-shadow: 0 0 2px #FFF inset;margin-top: 3px"><td type="hidden" style="display:none" >'+$("#carMortgageInfoId3").val()+'</td><td style="text-align: center">'+$("#custName2").val()+'</td><td style="text-align: center">'+$("#custIdNo2").val()+'</td><td style="text-align: center">'+$("#acceptConfirmDate2").val()+'</td><td style="text-align: center"><button class="btn btn-primary" onclick="delCarMortgageInfo(this);"><i class="fa fa-remove"></i>&nbsp;&nbsp;&nbsp;&nbsp;删除</button></td><td style="text-align: center"><button class="btn btn-primary" onclick="updCarMortgageInfo(this);"><i class="fa fa-edit"></i>&nbsp;&nbsp;&nbsp;&nbsp;修改</button></td></tr>';
    parent.$("#newAddCarMortgageInfoTr").append(str1);*/
    var ajax = new $ax(Feng.ctxPath + "/carMortgageInfo/saveMortgageInfo", function (data) {
        Feng.success("修改成功!");
    }, function (data) {
        Feng.error("修改失败!" + data.responseJSON.message + "!");
    });
    ajax.set("id",$("#carMortgageInfoId3").val());
    ajax.set("custName",$("#custName2").val());
    ajax.set("custIdNo",$("#custIdNo2").val());
    ajax.set("acceptConfirmDate",$("#acceptConfirmDate2").val());
    ajax.start();
    window.parent.carInfoVoLiClick();
    parent.layer.close(window.parent.layerIndex);
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if(status=='0'){
                    ...
                    var cretid = data.data1.certId;
                    if(cretid.substring(cretid.length-2,cretid.length-1)%2==0){//偶数为女
						$("#sex").val("1");
                        $("#sexName").val("女");
                    }else{//奇数为男
                        $("#sex").val("0");
                    ...
                    var year = cretid.substring(6,10);
                    var month = cretid.substring(10,12);
                    var day = cretid.substring(12,14);
                    var birthday = year+"-"+month+"-"+day;
                    $('#birthdayName').val(birthday);
                    ...
                    if(familyBookSubInfolist.length>0){
                           ...
                           for(var i =0;i<familyBookSubInfolist.length;i++){
                               if(familyBookSubInfolist[i].sex==0){
                                   var str1='<tr  style="background-color:#FFF;padding: 1px;box-shadow: 0 0 2px #FFF inset;margin-top: 3px"><td type="hidden" style="display:none" >'+familyBookSubInfolist[i].id+'</td><td style="text-align: center">'+familyBookSubInfolist[i].relationship+'</td><td style="text-align: center">'+familyBookSubInfolist[i].name+'</td><td style="text-align: center">'+familyBookSubInfolist[i].certId+'</td><td style="text-align: center">男</td><td style="text-align: center"><button class="btn btn-primary" onclick="delFamilyBookSubInfo(this);"><i class="fa fa-remove"></i>&nbsp;&nbsp;&nbsp;&nbsp;删除</button></td><td style="text-align: center"><button class="btn btn-primary" onclick="updFamilyBookSubInfo(this);"><i class="fa fa-edit"></i>&nbsp;&nbsp;&nbsp;&nbsp;修改</button></td><td style="display:none">'+idPicUrls+familyBookSubInfolist[i].bookPhotoUrl+'</td></tr>';
                               }else{
                                   var str1='<tr  style="background-color:#FFF;padding: 1px;box-shadow: 0 0 2px #FFF inset;margin-top: 3px"><td type="hidden" style="display:none" >'+familyBookSubInfolist[i].id+'</td><td style="text-align: center">'+familyBookSubInfolist[i].relationship+'</td><td style="text-align: center">'+familyBookSubInfolist[i].name+'</td><td style="text-align: center">'+familyBookSubInfolist[i].certId+'</td><td style="text-align: center">女</td><td style="text-align: center"><button class="btn btn-primary" onclick="delFamilyBookSubInfo(this);"><i class="fa fa-remove"></i>&nbsp;&nbsp;&nbsp;&nbsp;删除</button></td><td style="text-align: center"><button class="btn btn-primary" onclick="updFamilyBookSubInfo(this);"><i class="fa fa-edit"></i>&nbsp;&nbsp;&nbsp;&nbsp;修改</button></td><td style="display:none">'+idPicUrls+familyBookSubInfolist[i].bookPhotoUrl+'</td></tr>';
                               }
            ...
            success: function(data) {
                ...
                if(status=='0'){
                    ...
                    if(data.data.marryStatus==0||data.data.marryStatus==2){//已婚或再婚
                        ...
                        if(data.data.spouseSex==0){
                            $("#spouseSexName").val("男");
						}else if(data.data.spouseSex==1){
                            $("#spouseSexName").val("女");
                        }
...
function newAddCarInfo(){
    if($("#CarInfoId").val()==undefined||$("#CarInfoId").val()==""){
        Feng.info("请先保存车辆信息!")
	}else{
        var index = layer.open({
            type: 2,
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/carInfo/newAddCarInfo?CarInfoId='+$("#CarInfoId").val()
        });
        this.layerIndex = index;
	}
}
function AddCarInfo(){
    ...
    var ajax = new $ax(Feng.ctxPath + "/carTransferInfo/saveCarTransferInfo", function (data) {
       ...
       ajax.set("carInfoId",$("#CarInfoId1").val());
       ajax.set("carTransferInfoName",$("#carTransferInfoName").val());
       ajax.set("carTransferInfoCertId",$("#carTransferInfoCertId").val());
       ajax.set("carTransferInfoGetType",$("#carTransferInfoGetType").val());
       ajax.set("carTransferInfoRegDate",$("#carTransferInfoRegDate").val());
       ajax.start();
       window.parent.carInfoVoLiClick();
...
function newAddCarMortgageInfo(){
	if($("#CarInfoId").val()==undefined||$("#CarInfoId").val()==""){
        ...
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/carInfo/newAddCarMortgageInfo?CarInfoId='+$("#CarInfoId").val()
        });
        this.layerIndex = index;
	}
}
function AddCarMortgageInfo(){
    ...
    // var str1='<tr  style="background-color:#FFF;padding: 1px;box-shadow: 0 0 2px #FFF inset;margin-top: 3px"><td type="hidden" style="display:none" ></td><td style="text-align: center">'+$("#custName").val()+'</td><td style="text-align: center">'+$("#custIdNo").val()+'</td><td style="text-align: center">'+$("#acceptConfirmDate").val()+'</td><td style="text-align: center"><button class="btn btn-primary" onclick="delCarMortgageInfo(this);"><i class="fa fa-remove"></i>&nbsp;&nbsp;&nbsp;&nbsp;删除</button></td><td style="text-align: center"><button class="btn btn-primary" onclick="updCarMortgageInfo(this);"><i class="fa fa-edit"></i>&nbsp;&nbsp;&nbsp;&nbsp;修改</button></td></tr>';
    // parent.$("#newAddCarMortgageInfoTr").append(str1);
    var ajax = new $ax(Feng.ctxPath + "/carMortgageInfo/saveMortgageInfo", function (data) {
               Feng.success("新增成功!");
            }, function (data) {
        ...
        ajax.set("CarInfoId2",$("#CarInfoId2").val());
        ajax.set("custName",$("#custName").val());
        ajax.set("custIdNo",$("#custIdNo").val());
        ajax.set("acceptConfirmDate",$("#acceptConfirmDate").val());
        ajax.start();
    window.parent.carInfoVoLiClick();
    parent.layer.close(window.parent.layerIndex);
}
                ...
                if(status=='0'){
                    ...
                    if(carTransferInfolist.length>0){
                        ...
                        for(var i =0;i<carTransferInfolist.length;i++){
                            ...
                            var str1='<tr  style="background-color:#FFF;padding: 1px;box-shadow: 0 0 2px #FFF inset;margin-top: 3px"><td type="hidden" style="display:none" >'+carTransferInfolist[i].id+'</td><td style="text-align: center">'+carTransferInfolist[i].name+'</td><td style="text-align: center">'+carTransferInfolist[i].certId+'</td><td style="text-align: center">'+carTransferInfolist[i].getType+'</td><td style="text-align: center">'+ carTransferInfolist[i].regDate+'</td><td style="text-align: center"><button class="btn btn-primary" onclick="delCarTransferInfo(this);"><i class="fa fa-remove"></i>&nbsp;&nbsp;&nbsp;&nbsp;删除</button></td><td style="text-align: center"><button class="btn btn-primary" onclick="updCarTransferInfo(this);"><i class="fa fa-edit"></i>&nbsp;&nbsp;&nbsp;&nbsp;修改</button></td></tr>';
                    ...
                    if(carMortgageInfoList.length>0){
                        ...
                        for(var i =0;i<carMortgageInfoList.length;i++){
                           carMortgageInfoList[i].regDate=""?"":carMortgageInfoList[i].regDate.split(" ")[0];
                            var str1='<tr  style="background-color:#FFF;padding: 1px;box-shadow: 0 0 2px #FFF inset;margin-top: 3px"><td type="hidden" style="display:none" >'+carMortgageInfoList[i].id+'</td><td style="text-align: center">'+carMortgageInfoList[i].name+'</td><td style="text-align: center">'+carMortgageInfoList[i].certId+'</td><td style="text-align: center">'+carMortgageInfoList[i].regDate+'</td><td style="text-align: center"><button class="btn btn-primary" onclick="delCarMortgageInfo(this);"><i class="fa fa-remove"></i>&nbsp;&nbsp;&nbsp;&nbsp;删除</button></td><td style="text-align: center"><button class="btn btn-primary" onclick="updCarMortgageInfo(this);"><i class="fa fa-edit"></i>&nbsp;&nbsp;&nbsp;&nbsp;修改</button></td></tr>';
...
function setMasterSex() {
	var idNo = $("#masterCertId").val();
    if(idNo.substring(idNo.length-2,idNo.length-1)%2==0){//偶数为女
        $("#masterSex").val("1");
        $("#masterSexName").val("女");
    }else{//奇数为男
        $("#masterSex").val("0");
...
function setSubInfoSex() {
    var idNo = $("#familyBookSubInfoCertId1").val();
    if(idNo.substring(idNo.length-2,idNo.length-1)%2==0){//偶数为女
        $("#familyBookSubInfoSex1").val("1");
        $("#familyBookSubInfoSex1Name").val("女");
    }else{//奇数为男
        $("#familyBookSubInfoSex1").val("0");
        $("#familyBookSubInfoSex1Name").val("男");
    }
}

function setSubInfoSex2() {
    var idNo = $("#familyBookSubInfoCertId2").val();
    if(idNo.substring(idNo.length-2,idNo.length-1)%2==0){//偶数为女
        $("#familyBookSubInfoSex2").val("1");
        $("#familyBookSubInfoSex2Name").val("女");
    }else{//奇数为男
        $("#familyBookSubInfoSex2").val("0");
        $("#familyBookSubInfoSex2Name").val("男");
    }
}

function setSpouseSex() {
    if($("#spouseCertId").val()!=null&&$("#spouseCertId").val()!=""){
        var idNo = $("#spouseCertId").val();
        if(idNo.substring(idNo.length-2,idNo.length-1)%2==0){//偶数为女
            $("#spouseSex").val("1");
            $("#spouseSexName").val("女");
        }else{//奇数为男
    ...
    if($("#divorceCertId").val()!=null&&$("#divorceCertId").val()!=""){
        var idNo = $("#divorceCertId").val();
        if(idNo.substring(idNo.length-2,idNo.length-1)%2==0){//偶数为女
            $("#divorceSex").val("1");
            $("#divorceSexName").val("女");
        }else{//奇数为男
            $("#divorceSex").val("0");
            $("#divorceSexName").val("男");
        }
    }
}