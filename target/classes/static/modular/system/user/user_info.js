...
UserInfoDlg.onClickDept = function (e, treeId, treeNode) {
    ...
    //$("#deptid").attr("value", nodes);
    //$("#citySel").attr("value", ids.substring(1));
...
UserInfoDlg.hideDeptSelectTree = function () {
    ...
    var nodes = Feng.zTreeCheckedNodesName("treeDemo");

    $("#citySel").attr("value",  nodes);
...
UserInfoDlg.collectData = function () {
    this.set('id').set('account').set('sex').set('password').set('avatar')
        .set('email').set('name').set('birthday').set('rePassword').set('phone');
    var nodes = Feng.zTreeCheckedNodes("treeDemo");
    // var ids = "";
    // for (var i = 0, l = nodes.length; i < l; i++) {
    //     ids += "," + nodes[i].id;
    // }
    this.userInfoData['deptid'] = nodes;
};
...
$(function () {
    ...
    var userId = UserInfoDlg.get("id");
    if(userId == null || userId ==""){
        userId = "0";
    }
    var ztree = new $ZTree("treeDemo", "/dept/deptTreeListByUserId/"+ userId);
    ztree.settings={
        check: {
            enable: true,
            chkboxType: {
        ...
        callback : {
            onClick : UserInfoDlg.onClickDept
        }
    };
